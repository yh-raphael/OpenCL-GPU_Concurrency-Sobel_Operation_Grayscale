//	120210400 조 용 현
//  GPU_Concurrency.cl						// Vector4 type variable + Loop unrolling + Shared memory.
//
//  Written for CSEG437_CSE5437
//  Department of Computer Science and Engineering
//  Copyright © 2021 Sogang University. All rights reserved.

#define INPUT(x, y)				input_data[n_columns * (y) + (x) + _offset_]	// [HW3] // 
...
__kernel void GPU_Concurrency(
	...
	int filter_x_width, int filter_y_width, __local uchar4* shared_mem, unsigned int _offset_)
{
...
//	offset = row * n_columns + column;
	offset = row * n_columns + column + _offset_;
...
//		offset = row * n_columns + column - HFS_5;
		offset = row * n_columns + column - HFS_5 + _offset_;
		uchar intensity = (uchar)(input_data[offset].x * gray[0]) + (uchar)(input_data[offset].y * gray[1])
						+ (uchar)(input_data[offset].z * gray[2]);
...
//		offset = (row - HFS_5) * n_columns + column;
		offset = (row - HFS_5) * n_columns + column + _offset_;
		uchar intensity = (uchar)(input_data[offset].x * gray[0]) + (uchar)(input_data[offset].y * gray[1])
						+ (uchar)(input_data[offset].z * gray[2]);
...
//			offset = (row - HFS_5) * n_columns + column - HFS_5;
			offset = (row - HFS_5) * n_columns + column - HFS_5 + _offset_;
			uchar intensity = (uchar)(input_data[offset].x * gray[0]) + (uchar)(input_data[offset].y * gray[1])
							+ (uchar)(input_data[offset].z * gray[2]);
